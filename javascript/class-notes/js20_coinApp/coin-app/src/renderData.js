export const renderCoins = async (coin) => {
  const { price, name, iconUrl, change, symbol, rank } = coin;
  const coinsUl = document.querySelector(".coins");
  ...
  newCoinLi.innerHTML = `
    ...
    `;
  //?oluşan li elementini ul listesine bağladık
  coinsUl.append(newCoinLi);

  //? Sil butonu için event tanımlanması
